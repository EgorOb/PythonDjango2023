...
def filtering_category(database: dict,
    ...
    if category_key is not None:
        result = [product for product in database.values() if product[
            "category"] == category_key]  # TODO При помощи фильтрации в list comprehension профильтруйте товары по категории. Или можете использовать
        # обычный цикл или функцию filter
    else:
        result = [product for product in
                  database.values()]  # TODO Трансформируйте database в список словарей
